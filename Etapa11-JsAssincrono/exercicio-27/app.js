...
setTimeout(()=>{
  console.log('executando uma mensagem em código assíncrono')
},2000)
...
*/
const funcRecebeValores = (item1, item2, item3)=> {
...
booksBox.addBooks = booksQuantity =>{
  ...
  if( booksBox.booksIn + booksQuantity > booksBox.spaces){
      ...
      const bookOrSingular = avaliableSpaces === 1 ? 'livro' : 'livros'
      return`Só ${fitPluralOrSingular} mais ${avaliableSpaces} ${bookOrSingular}`
  }

 booksBox.booksIn += booksQuantity
 const bookPluralOrSingular = booksBox.booksIn === 1 ? 'livros' : 'livros'
